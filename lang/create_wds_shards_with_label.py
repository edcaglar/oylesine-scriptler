...
def write_shards(
    ...
    #!!!!!!!!!
    # find all audio files with expected labels
    with open(labels_path, "r") as file:
        labels = file.read().splitlines()

    audio_files = []
    for label in labels:
        audio_files.extend([f for f in voxlingua_folder_path.glob("{}/*.wav".format(label))])
    audio_files = sorted(audio_files)
    # audio_files = sorted([f for f in voxlingua_folder_path.rglob("*.wav")])

    # create tuples (unique_sample_id, language _id, path_to_audio_file, duration)
    data_tuples = []
...
parser.add_argument(
    "--labels_path",
    type=pathlib.Path,
    help="Wanted labels to create shards in txt format",
)
